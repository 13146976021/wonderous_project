      ...
      child: SizedBox(
        ...
        child: Stack(
          children: [
            ...
            BottomCenter(
                child: Container(
                    height: barSize - barTopPadding, color: Colors.red)),
            ClipRect(
              child: OverflowBox(
                alignment: Alignment.topCenter,
                maxHeight: circleSize,
                child: _AnimatedCircleWithText(
                  titles: titles,
                  index: index,
                ),
              ),
...
class _AnimatedCircleWithTextState extends State<_AnimatedCircleWithText>
  ...
  late final _anim =
      AnimationController(vsync: this, duration: $styles.times.med)..forward();
  ...
  Widget _buildCircularText(String title) {
    final textStyle = $styles.text.monoTitleFont
        .copyWith(fontSize: 22 * $styles.scale, color: $styles.colors.accent1);

    return CircularText(position: CircularTextPosition.inside, children: [
      TextItem(
